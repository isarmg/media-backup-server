...
```bash
grep ' media-backup-server-0.3.6-x86_64-unknown-linux-gnu.tar.gz$' SHA256SUMS \
  | sha256sum --check -
tar -xzf media-backup-server-0.3.6-x86_64-unknown-linux-gnu.tar.gz
...
安装只允许创建缺失的 `/opt/isarmg/media-backup/releases/0.3.6`，不会覆盖或复用。同版本重装应先按
