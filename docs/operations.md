...
./scripts/build-server-release.sh \
  ...
  "$revision" "$PWD/dist"
./scripts/test-deployment.sh "$PWD/dist/media-backup-server-0.3.10-x86_64-unknown-linux-gnu.tar.gz"
```
...
```bash
grep ' media-backup-server-0.3.10-x86_64-unknown-linux-gnu.tar.gz$' SHA256SUMS \
  | sha256sum --check -
tar -xzf media-backup-server-0.3.10-x86_64-unknown-linux-gnu.tar.gz
...
安装只允许创建缺失的 `/opt/isarmg/media-backup/releases/0.3.10`，不会覆盖或复用。同版本重装应先按
...
服务端软件版本是 `0.3.10`，但数据库合同独立保持不变：`product_metadata` 必须精确为
...
管理端只呈现“备份实例”：`POST /api/v2/admin/instances` 只接收实例名称，并在同一数据库事务中创建自动分配的
...
当前 Server Rust 固定 Foundation `=0.8.1` / `85348eb99cbea7798a2a3f8ea55baf0179708322`；八个 Web 包使用
