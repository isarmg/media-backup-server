...
本文按 Media Backup `0.3.16` 当前工作树盘点 Server、React 管理 Web、共享 Rust Client、Android、iOS、
...
| MED-R-015 | 安装 no-clobber 固定 `/opt/isarmg/media-backup/releases/0.3.16`，环境 0600 | `setup-wsl.sh`、deployment tests | 保障 | 高 | 同版本覆盖会让运行内容不可追溯，Secret 权限过宽 | 首装/二次安装、concurrent、mode/owner |
...
以及鉴权 Range/HEAD/ETag 内容读取；具体协议与验证见 [图库 API 扩展](https://github.com/isarmg/media-backup-server/blob/v0.3.16/docs/gallery-api.md)。
