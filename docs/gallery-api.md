...
`/v2` 图库接口提供时间线、分页快照、同步事件、资源读取和派生预览。
全部接口经过设备/API key 鉴权，并以实际 account_id 限定查询；管理员会话不是媒体访问凭据。

## 全库时间线

`GET /v2/timeline` 接受 cursor、limit、trashed、favorite、archived、album_id、tag_id，以及以下筛选参数：
...
已经读过资产的更新、永久删除都由随后重放的事件补齐。同步游标只能在对应事件成功应用后推进。
