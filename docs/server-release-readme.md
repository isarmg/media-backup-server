# Media Backup Server 0.3.21 发行包使用手册

本文档只面向已经构建完成的 `media-backup-server-0.3.21-x86_64-unknown-linux-gnu.tar.gz`
正式发行包。它不是源码构建指南，也不描述 Android、iOS 或开发环境。本文出现的命令、路径和文件均以
当前 `0.3.21` 发行包的实际内容为准。
...
- 安装器只接受全新的目标。它不会覆盖、合并或修补已有 `0.3.21` 发行目录，也不会覆盖已有 systemd
  unit。
...
```text
media-backup-server-0.3.21-x86_64-unknown-linux-gnu/
...
```bash
archive='media-backup-server-0.3.21-x86_64-unknown-linux-gnu.tar.gz'
...
4. 把经过校验的完整发行复制到 `/opt/isarmg/media-backup/releases/0.3.21`，设为 root 所有，并再次
...
```text
/opt/isarmg/media-backup/releases/0.3.21/   # root 拥有的不可变发行
...
```text
/opt/isarmg/media-backup/releases/0.3.21/bin/media-backup-server serve-release /opt/isarmg/media-backup/releases/0.3.21
```
...
```bash
/opt/isarmg/media-backup/releases/0.3.21/scripts/verify-server-wsl.sh
```
...
- [ ] 发行位于固定 `/opt/isarmg/media-backup/releases/0.3.21`，未被在线编辑。
...
以上清单全部通过，才表示当前 `0.3.21` Server 发行具备上线条件。
