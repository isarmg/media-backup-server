...
本仓库为 Media Backup Server `0.3.13` 开发版，仅包含 Rust 服务端、协议和管理 Web。
...
本项目只实现当前版本。服务端创建并只接受 Media Backup `0.3.0` 数据格式（Schema revision 4），软件版本 `0.3.13`；移动端状态合同由 Client 仓库规定。
...
恰为 `{authenticated,user_id,username,role:"admin",csrf_token}`。移动客户端不使用账户密码：管理员只填写实例名称，Server 会在同一事务中建立自动分配的隔离存储、100 GiB 默认配额、客户端实例和唯一长期授权码，无需先创建用户；路径和配额之后可在详情页调整。把授权码交给 Android/iOS 即可配对；授权码以信封密文保存且可查看/轮换，轮换后客户端必须重新配对。这里只支持新的当前 Schema，不包含旧密码 bootstrap 或进程内迁移兼容。
