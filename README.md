...
本仓库为 Media Backup Server `0.3.7` 开发版，仅包含 Rust 服务端、协议和管理 Web。
...
本项目只实现当前版本。服务端继续创建并接受 Media Backup `0.3.0` 数据格式（Schema revision 3），软件版本 `0.3.7` 不改变该数据格式；移动端状态合同由 Client 仓库规定。
不属于当前身份的数据库和凭据一律拒绝，产品仓库也不提供
迁移、备份和恢复命令。离线任务只能由 `sarmg-upgrade` 的精确支持矩阵执行；该工具目前尚不支持
Media Backup `0.3.0` / revision 3，不能用旧适配器处理当前状态。
