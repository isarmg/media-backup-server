...
本仓库为 Media Backup Server `0.3.6` 开发版，仅包含 Rust 服务端、协议和管理 Web。
...
本项目只实现当前版本。服务端继续创建并接受 Media Backup `0.3.0` 数据格式（Schema revision 3），软件版本 `0.3.6` 不改变该数据格式；移动端状态合同由 Client 仓库规定。
